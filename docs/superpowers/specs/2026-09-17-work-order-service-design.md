...
同一服务目录和优先级只能存在一条启用策略。该组合唯一性由领域服务在创建工单和策略验收中检查，不依赖 SQLite 复合索引。
...
5. 无处理记录不能提交解决；
6. 处理人不能复核自己的工单；
7. SLA 达标、未完成超时和关闭超时计算准确；
8. 通用 CRUD 不能修改工单状态、处理人或事件历史；
9. 事件或审计失败不留下部分工单数据；
10. 插件通过生产 `PluginRegistry` 和 `loadRuntimeBlueprint` 加载并注册全部贡献；
11. UI 描述符只使用声明槽位且为纯数据；
12. 种子数据确定、引用有效且匿名；
13. 领域包、蓝图、桌面底座和 PowerShell 总回归全部通过；
14. 无依赖、构建、数据库、日志或敏感凭据产物被意外提交。
