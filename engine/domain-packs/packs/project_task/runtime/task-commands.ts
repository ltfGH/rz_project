...
export function startTask(request:VersionedTaskRequest,context:ProjectContext):TaskResult {const permission='project_tasks.start';context.requirePermission(context.actor,permission);const row=readTask(request.taskId,context),project=readTaskProject(row,context);assertActive(project.status);assertAssignee(row,context);return transition(row,context,request.expectedVersion,permission,'task_started','pending','in_progress',null,'',[],false);}
export function addTaskProgress(request:AddTaskProgressRequest,context:ProjectContext):TaskResult {const permission='project_tasks.add_progress';context.requirePermission(context.actor,permission);const row=readTask(request.taskId,context),project=readTaskProject(row,context);assertActive(project.status);assertAssignee(row,context);const note=required(request.note,'note');return transition(row,context,request.expectedVersion,permission,'task_progress_added','in_progress','in_progress',note,'progress_note=?,',[note],false);}
export function submitTaskReview(request:VersionedTaskRequest,context:ProjectContext):TaskResult {const permission='project_tasks.submit';context.requirePermission(context.actor,permission);const row=readTask(request.taskId,context),project=readTaskProject(row,context);assertActive(project.status);assertAssignee(row,context);return transition(row,context,request.expectedVersion,permission,'task_submitted','in_progress','pending_review',null,'submitted_at=?,',[context.now().toISOString()],false);}
